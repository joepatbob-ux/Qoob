...
//  Qoob
//
//  The app's non-game screens: the Settings sheet (control scheme and feedback
//  preferences, presented from the HUD gear button) and the level builder it opens.
//  Both use standard SwiftUI containers, which adopt Liquid Glass automatically on
//  iOS 26+.
//
//  The builder shares this file rather than having one of its own because the
//  project references every source file explicitly — there's no synchronized group,
//  so a new file needs a change to the Xcode project itself. Worth moving out the
//  moment that's done.
//
