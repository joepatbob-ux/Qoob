...
//
//  Authored houses live in HouseBlueprint.swift and the RNG in SeededGenerator.swift;
//  both produce or feed the same `Level` this file generates, so there is one shape of
//  house in the game however it was made.
//
...
struct Level {
    ...
    static func wallBetween(_ a: HouseRoom, _ b: HouseRoom) -> [GridCell]? {
    ...
    static func connected(from start: GridCell, within cells: Set<GridCell>) -> Set<GridCell> {
    ...
    static func nearestFloor(to cell: GridCell, in floor: Set<GridCell>) -> GridCell {
