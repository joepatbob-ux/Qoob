...
enum ProceduralTextures {
    ...
    static func roomFloor(for environment: Environment, _ appearance: Appearance) -> UIImage {
        ...
        case .sandpit:    return cached(key) { sand(appearance) }
        case .patio:      return cached(key) { paving(appearance) }
        }
    }
    ...
    /// Sandpit: warm sand. No structure at all — just grain at two scales, dense fine
    /// speckle over broad soft drifts, which is what separates sand from a flat fill.
    private static func sand(_ appearance: Appearance) -> UIImage {
        renderer().image { rctx in
            let ctx = rctx.cgContext
            ctx.setFillColor(pick(appearance,
                                  light: UIColor(red: 0.86, green: 0.77, blue: 0.56, alpha: 1),
                                  dark: UIColor(red: 0.44, green: 0.37, blue: 0.26, alpha: 1)).cgColor)
            ctx.fill(fullRect)

            var rng = SeededGenerator(seed: 0x5A_4D00)
            // Drifts: broad, very faint, so the surface undulates rather than sitting flat.
            for _ in 0..<90 {
                let x = CGFloat(rng.next() % UInt64(px))
                let y = CGFloat(rng.next() % UInt64(px))
                let r = 12 + CGFloat(rng.next() % 34)
                ctx.setFillColor(UIColor(white: rng.next() % 2 == 0 ? 1 : 0,
                                         alpha: 0.030).cgColor)
                ctx.fillEllipse(in: CGRect(x: x - r, y: y - r, width: 2 * r, height: 2 * r))
            }
            speckle(ctx, &rng, count: 7000, radius: 0.8, maxAlpha: 0.10)
        }
    }

    /// Patio: cut stone slabs. Larger and cooler than the kitchen's tile, and jointed
    /// on the same two edges so a repeat runs continuously.
    private static func paving(_ appearance: Appearance) -> UIImage {
        renderer().image { rctx in
            let ctx = rctx.cgContext
            ctx.setFillColor(pick(appearance,
                                  light: UIColor(red: 0.70, green: 0.68, blue: 0.64, alpha: 1),
                                  dark: UIColor(red: 0.33, green: 0.32, blue: 0.31, alpha: 1)).cgColor)
            ctx.fill(fullRect)

            var rng = SeededGenerator(seed: 0x9A_7E00)
            // Stone is blotchy: bigger, stronger mottle than glazed tile.
            for _ in 0..<120 {
                let x = CGFloat(rng.next() % UInt64(px))
                let y = CGFloat(rng.next() % UInt64(px))
                let r = 6 + CGFloat(rng.next() % 22)
                ctx.setFillColor(UIColor(white: rng.next() % 2 == 0 ? 1 : 0,
                                         alpha: 0.055).cgColor)
                ctx.fillEllipse(in: CGRect(x: x - r, y: y - r, width: 2 * r, height: 2 * r))
            }
            speckle(ctx, &rng, count: 2600, radius: 1.0, maxAlpha: 0.09)

            grout(ctx, width: 10,
                  color: pick(appearance,
                              light: UIColor(red: 0.55, green: 0.53, blue: 0.50, alpha: 1),
                              dark: UIColor(red: 0.22, green: 0.21, blue: 0.20, alpha: 1)))
        }
    }
